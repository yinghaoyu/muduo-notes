...
string uptime(Timestamp now, Timestamp start, bool showMicroseconds)
{
  ...
  if (showMicroseconds)
  {
    ...
    snprintf(buf, sizeof buf, "%d days %02d:%02d:%02d.%06d", days, hours, minutes, seconds % 60, microseconds);
  }
  else
  {
...
using namespace convey::inspect;
...
string ProcessInspector::overview(HttpRequest::Method, const Inspector::ArgList &)
{
  ...
  stringPrintf(&result, "%.3f MiB, ", static_cast<double>(getLong(procStatus, "VmSize:")) / 1024.0);
