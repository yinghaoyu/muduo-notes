...
{
  if (::getenv("CONVEY_LOG_TRACE"))
    return Logger::TRACE;
  else if (::getenv("CONVEY_LOG_DEBUG"))
    return Logger::DEBUG;
  else
    return Logger::INFO;
}
...
void Logger::Impl::formatTime()
{
  ...
  if (seconds != t_lastSecond)
  {
    ...
    int len = snprintf(t_time, sizeof(t_time), "%4d%02d%02d %02d:%02d:%02d", tm_time.tm_year + 1900, tm_time.tm_mon + 1, tm_time.tm_mday, tm_time.tm_hour,
